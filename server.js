...
// 🔧 TEMPORÁRIO: Hardcode da chave Stripe para teste
// Depois que funcionar, volte para: const stripe = require('stripe')(process.env.STRIPE_SECRET_KEY)
...
async function enviarWhatsApp(numero, mensagem, tipo = 'geral') {
  ...
  if (!ULTRAMSG_INSTANCE || !ULTRAMSG_TOKEN) {
    console.log('⚠️ UltraMsg não configurada no .env')
    return false
  }
    ...
    <html>
      <head>
        ...
        <style>
          ...
          h1 { color: #2c3e50; }
          .card { background: #ecf0f1; padding: 20px; margin: 10px; border-radius: 10px; display: inline-block; }
      ...
      <body>
        ...
        <p>Sistema operacional com segurança LGPD</p>
        <div class="card">
          <h2>📋 Endpoints Disponíveis</h2>
          <div class="endpoint">GET / - Home</div>
          <div class="endpoint">POST /api/webhook/triagem - Triagem</div>
          <div class="endpoint">GET /dashboard - Dashboard</div>
          <div class="endpoint">GET /fila - Ver fila</div>
          <div class="endpoint">GET /atendimentos - Todos atendimentos</div>
          <div class="endpoint">GET /atendimento/:id - Detalhes do paciente</div>
          <div class="endpoint">POST /decisao/:id - Aprovar/Recusar</div>
          <div class="endpoint">POST /prontuario/:id - Salvar prontuário</div>
          <div class="endpoint">POST /gerar-receita/:id - Emitir receita</div>
          <div class="endpoint">GET /receita/:id - Ver receita</div>
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      line_items: [{
        price_data: {
          ...
          product_data: { name: process.env.PRODUCT_NAME || 'Consulta Assíncrona' },
...
app.post('/api/webhook/triagem', async (req, res) => {
  ...
  if (!req.body.paciente || !req.body.triagem) {
  ...
  if (doencaTexto.includes('has') || doencaTexto.includes('hipertensao')) {
    ...
    doencaCodificada = 'DLP'
  } else if (doencaTexto.includes('hipotireoidismo')) {
    doencaCodificada = 'HIPOTIREOIDISMO'
  }

  const doencaValida = ['HAS', 'DM', 'DLP', 'HIPOTIREOIDISMO'].includes(doencaCodificada)

  let diasVencidos = null
  if (triagem?.dataVencimentoReceita) {
    const data = new Date(triagem.dataVencimentoReceita)
    if (!isNaN(data.getTime())) {
      const hoje = new Date()
      hoje.setHours(0, 0, 0, 0)
      data.setHours(0, 0, 0, 0)
      diasVencidos = Math.floor((hoje - data) / (1000 * 3600 * 24))
    }
  }

  const receitaValida = diasVencidos !== null && diasVencidos >= 0 && diasVencidos <= 180

  const sinaisAlerta = triagem?.sinaisAlerta === true ||
    triagem?.sinaisAlerta === 'true' ||
    triagem?.sinaisAlerta === 'SIM' ||
    triagem?.sinaisAlerta === 'sim' ||
    triagem?.sinaisAlerta === 'S'
  ...
  if (elegivel && paciente.telefone && validarTelefone(paciente.telefone) && !atendimento.whatsappEnviado.triagem) {
    const mensagemWhats = `Olá ${paciente.nome}! ✅ Seu atendimento foi aprovado.\n\n🔗 Link para pagamento: ${BASE_URL}/api/create-payment/${id}\n\n💰 Valor: R$ 69,90`
    const enviado = await enviarWhatsApp(paciente.telefone, mensagemWhats, 'triagem')
    if (enviado) {
      atendimento.whatsappEnviado.triagem = true
      db.saveAtendimentos(lista)
    }
  }
...
app.get('/success', async (req, res) => {
  try {
    ...
    if (at && !at.whatsappEnviado.pagamento) {
      at.pagamento = true
      ...
      if (at.paciente?.telefone) {
        ...
        if (validarTelefone(telefone)) {
          ...
          const enviado = await enviarWhatsApp(telefone, mensagemWhats, 'pagamento')
          if (enviado) {
            at.whatsappEnviado.pagamento = true
          }
        }
      }
      db.saveAtendimentos(lista)
...
// ========================
// 👨‍⚕️ BUSCAR ATENDIMENTO POR ID (COM DESCRIPTOGRAFIA)
// ========================
...
app.post('/decisao/:id', async (req, res) => {
  ...
  if (decisao === 'APROVAR') {
    ...
    if (at.paciente?.telefone) {
      const telefone = decrypt(at.paciente.telefone)
      if (validarTelefone(telefone)) {
        await enviarWhatsApp(telefone, `✅ Seu atendimento foi APROVADO pelo médico! Em breve você receberá a receita.`, 'aprovacao')
      }
    }
    ...
    if (at.paciente?.telefone) {
      const telefone = decrypt(at.paciente.telefone)
      if (validarTelefone(telefone)) {
        await enviarWhatsApp(telefone, `❌ Seu atendimento foi RECUSADO pelo médico. Motivo: ${at.motivo || 'Não atende aos critérios'}`, 'recusa')
      }
    }
  }
...
// ========================
// 📄 GERAR RECEITA (INTEGRAÇÃO MEMED)
// ========================
...
// ========================
// 👁️ VISUALIZAR RECEITA (PÁGINA PÚBLICA)
// ========================
      ...
      <body>
        <div class="receita">
          ...
          <div class="medico">
            ...
            <p><strong>CRM:</strong> ${process.env.MEDICO_NUMERO} ${process.env.MEDICO_UF}</p>
            <p><strong>Data:</strong> ${new Date(receita.emitidaEm).toLocaleString()}</p>
            <div class="assinatura">_________________________</div>
            <p style="font-size: 12px;">Assinatura digital do médico</p>
          </div>
        </div>
      </body>
    </html>
...
app.post('/api/test-whatsapp', async (req, res) => {
  ...
  const resultado = await enviarWhatsApp(telefone, mensagem, 'teste')
