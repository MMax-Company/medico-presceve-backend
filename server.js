...
// ========================
const REQUIRED_ENV = ['ENCRYPTION_KEY', 'JWT_SECRET', 'STRIPE_SECRET_KEY']
...
const db = {
  ...
  async deletarAtendimento(id) {
    ...
    if (fs.existsSync(file)) {
      fs.unlinkSync(file)
      console.log(`🗑️ Atendimento deletado: ${id}`)
    }
  }
}
...
app.post('/api/webhook/triagem', async (req, res) => {
  try {
    ...
    const doencasElegiveis = ['has', 'hipertensao', 'diabetes', 'dm', 'hipotireoidismo', 'dislipidemia', 'dlp']
...
app.post('/api/decisao/:id', auth, async (req, res) => {
  try {
    ...
    if (at && at.paciente_telefone) {
      ...
      if (decisao === 'APROVAR') {
        const msg = `✅ Ótimas notícias, ${nome}!\n\n🎉 Sua receita foi APROVADA!\n\n📋 Número: ${req.params.id.substring(0, 8)}\n\n🏥 A receita digital será enviada em breve.\n\nDoctor Prescreve - Cuidando da sua saúde`
        await enviarWhatsApp(telefone, msg)
      } else {
        const msg = `❌ Infelizmente, ${nome}, sua receita foi RECUSADA.\n\n📋 Número: ${req.params.id.substring(0, 8)}\n\n🏥 Procure um atendimento presencial para renovar sua receita.\n\nDoctor Prescreve - Saúde sempre`
        await enviarWhatsApp(telefone, msg)
      }
    }
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        @media (max-width: 768px) {
            ...
            .painel-header h1 { font-size: 20px; }
        }
    </style>
</head>
<body>
    ...
    <div id="painel">
        <div class="container">
            ...
            <div class="filtros">
                ...
                <button class="filtro-btn" onclick="filtrar('inelegiveis')">⚠️ Inelegíveis</button>
            </div>
    ...
    <script>
        ...
        function renderizarAtendimentos() {
            ...
            } else if (filtroAtual === 'inelegiveis') {
                filtrados = filtrados.filter(a => a.status === 'INELEGIVEL');
            }
            ...
            let html = '<tr><thead><tr><th>ID</th><th>Paciente</th><th>Doença</th><th>Status</th><th>Pagamento</th><th>Ações</th></tr></thead><tbody>';
        ...
        async function verDetalhes(id) {
            try {
                ...
                const detalhes = 
                    ...
                    '📅 Data: ' + new Date(a.criado_em).toLocaleString();
                alert(detalhes);
            } catch(e) {
