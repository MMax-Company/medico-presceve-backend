...
<body>
    <div id="login" class="login-container">
        <div class="login-card">
            ...
            <p id="erro" style="color:red;margin-top:15px;display:none">Senha incorreta!</p>
        </div>
    </div>
    ...
    <script>
        ...
        async function carregarEstatisticas(){
            try{
                ...
                document.getElementById('stats').innerHTML='<div class="stat-card"><div class="stat-number">'+stats.total+'</div><div>📋 Total</div></div><div class="stat-card"><div class="stat-number">'+stats.elegiveis+'</div><div>✅ Elegíveis</div></div><div class="stat-card"><div class="stat-number">'+stats.pagos+'</div><div>💰 Pagos</div></div><div class="stat-card"><div class="stat-number">'+stats.naFila+'</div><div>⏳ Na Fila</div></div>';
            }catch(e){console.error(e);}
        }
        
        async function carregarAtendimentos(){
            try{
                const res=await fetch(API_URL+'/api/atendimentos',{headers:{'Authorization':'Bearer '+token}});
        ...
        function renderizarAtendimentos(){
            ...
            if(filtroAtual==='fila') filtrados=filtrados.filter(a=>a.pagamento&&a.status==='FILA');
            else if(filtroAtual==='aprovados') filtrados=filtrados.filter(a=>a.status==='APROVADO');
            else if(filtroAtual==='recusados') filtrados=filtrados.filter(a=>a.status==='RECUSADO');
            ...
            for(const a of filtrados){
                ...
                html+='<tr><td><code>'+a.id.substring(0,8)+'</code></td><td><strong>'+(a.paciente_nome||'N/A')+'</strong></td><td>'+(a.doencas||'N/A')+'</td><td><span class="status-badge '+statusClass+'">'+(a.status||'PENDENTE')+'</span></td><td>'+(a.pagamento?'✅ Pago':'⏳ Pendente')+'</td><td><button class="btn btn-info" onclick="verDetalhes(\''+a.id+'\')">Ver</button>'+(a.status==='FILA'?'<button class="btn btn-primary" onclick="aprovar(\''+a.id+'\')">Aprovar</button><button class="btn btn-danger" onclick="recusar(\''+a.id+'\')">Recusar</button>':'')+'</td></tr>';
            }
            html+='</tbody></table>';
        ...
        async function verDetalhes(id){
            try{
                const res=await fetch(API_URL+'/api/atendimento/'+id,{headers:{'Authorization':'Bearer '+token}});
        ...
        async function aprovar(id){
            ...
            try{
                await fetch(API_URL+'/api/decisao/'+id,{method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+token},body:JSON.stringify({decisao:'APROVAR'})});
        ...
        async function recusar(id){
            ...
            try{
                await fetch(API_URL+'/api/decisao/'+id,{method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+token},body:JSON.stringify({decisao:'RECUSAR'})});
